                                 ...
                                 arraySlice1d< real64, USD2 > const & vapourComposition )
{
  ...
  for( localIndex iterationCount = 0; iterationCount < maxProbeIterations; ++iterationCount )
  {
    ...
    // Only update K for components in the feed; fugacityRatios is undefined elsewhere.
    for( integer const ic : presentComponents )
    {
                                     ...
                                     arraySlice1d< real64, USD2 > const & vapourComposition )
{
  ...
  for( integer recoveryPass = 0; recoveryPass < numRecoveryPasses && !converged; ++recoveryPass )
  {
    if( recoveryPass > 0 )
    {
      ...
      if( recoveryPass == 6 )
      {
        // Newton-style update of the Wilson reference pressure using finite differences on the
        // same short successive-substitution residual as probeEquilibriumResidual (fugacity still
        // at the true cell pressure). The residual at the baseline must use that probe, not the
        // main-loop error, so f, f' and the Newton step P - f/f' are consistent.
        ...
        real64 const errMid = negative_two_phase_flash_internal::probeEquilibriumResidual(
          numComps, pressure, pressure, temperature, composition, componentProperties, flashData,
        ...
        if( LvArray::math::abs( denom ) > flashTolerance * flashTolerance + pressure * LvArray::NumericLimits< real64 >::epsilon )
        {
          wilsonReferencePressure = pressure - errMid * ( wPlus - wMinus ) / denom;
        }
        else
        {
          wilsonReferencePressure = ( errPlus < errMinus ) ? wPlus : wMinus;
        }
        wilsonReferencePressure = LvArray::math::min( LvArray::math::max( wilsonReferencePressure, 0.8 * pressure ), 1.2 * pressure );
      }
